...
- **Attempts:**
  ...
  - STANDARDZED on the `webhook@<id>` prefix for all job records in the `AGENT_CI_BRIDGE_JOBS` KV.
...
- **Console UI:** Refactored `JobsPage` to use the correct `AGENT_CI_BRIDGE_JOBS` binding, added columns for "Type" (Local vs. GitHub) and "SHA", and improved formatting.
