...
We implemented a pull-based architecture for the OA-1 runner where the worker container fetches its own job metadata and secrets directly from the GitHub API. This mirrors the official GitHub runner's security model and reduces the Bridge's role to a lightweight orchestrator and token provider.
...
- **Attempts:**
  ...
  - **Implementation:**
    - Updated `oa-1-bridge` to extract GitHub metadata (Job ID, Repo) and provide a `GITHUB_TOKEN`.
    - Updated `oa-1-runner` to inject these as `ACTIONS_RUNTIME_TOKEN`, `GITHUB_JOB_ID`, and `GITHUB_REPO`.
